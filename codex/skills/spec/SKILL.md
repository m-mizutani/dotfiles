...
ユーザーに問う箇所は、会話で尋ねるだけでなく **spec 本文の該当位置に `> ⚠️ **要判断**: ...` の引用ブロックを置いて強調表示する**（書式は「統合specファイルの作成」を参照）。会話だけで問うと、spec を後から読む人にはどこが未確定なのか分からない。
...
**全体**
- [ ] ユーザーの判断が必要な箇所が `> ⚠️ **要判断**: ...` の引用ブロックで表記され、**決めること・選択肢・各選択肢を採った場合の帰結・現時点の推奨**が書かれているか。判断が必要な箇所がないなら、このブロックが1つも残っていないか
...
- **ユーザーの判断が必要な箇所は `> ⚠️ **要判断**: ...` で始まる引用ブロックで表記する。** 引用ブロックと `⚠️` により、本文の地の文から区別できる状態にする。書く場所は判断が関わる章の該当位置（例: 保存先の選択なら「2. 基本設計 → 状態の持ち方」の直後）で、章末や文書末にまとめない。ブロックの中には **①何を決める必要があるのか ②選択肢（named な案として） ③各選択肢を採った場合の帰結 ④現時点での推奨と理由** を書く。ユーザーの判断が出たらこのブロックを削除し、確定した内容を本文と「3. 設計判断」へ反映する
...
> この「要判断」ブロックは、ユーザーの判断が必要な箇所に置く。置き場所は判断が関わる章の該当位置であり、この3章に限らない。判断が確定したらブロックを削除し、確定内容を本文と上の設計判断リストへ反映する。判断が必要な箇所がなければブロックごと削除する。
...
2. **ユーザーにレビューを依頼し、ここで停止する（必須）**: specを提示した状態でレビューを依頼し、ユーザーから明示的な承認を得るまで Phase 2 に進まない。「要判断」ブロックが残っている場合は、レビュー依頼の文でも**どの章のどの点について判断が必要かを一覧で示す**（spec 本文のブロックと重複してよい）。ただし、その後に `$implement` または対象と実装内容を明示した実装・続行・完了の指示を受けた場合は、その指示を承認として扱い、停止せず実装へ進む。

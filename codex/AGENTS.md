...
This file collects cross-project guidelines for Codex. It is intentionally free
of repository-specific names, paths, package layouts, environment variables, and
tool/framework choices. Project-specific guidance lives in each repository's
`AGENTS.md`; this file holds the rules that apply regardless of which project is
in front of you.
...
- What to include: pull requests, issues, commits and branches on the hosting service, CI runs and their failing jobs, deployed or preview environments, and any external document, ticket, or page created or updated during the work
...
- **Separate every bare URL from the surrounding text with a space on both sides.** Terminals auto-link URLs by scanning to the next whitespace, so an adjacent closing parenthesis, bracket, quote, comma, or period (half-width or full-width) is absorbed into the link and the resulting URL is wrong
...
- **Before any write operation (file edit / creation / deletion / git mutation), confirm the path you are about to touch is under the current worktree's root.** If a path resolves outside the current working tree, STOP — do not write to it
...
2. **Concepts that already exist and are known in this project/repository** — identifiers, file/package/module names, config keys, and terms defined in its docs, spec, or `AGENTS.md`
...
- **Do NOT add `Co-Authored-By` trailers (or any other co-author attribution) to commit messages, and do NOT append attribution footers (e.g. `Generated with Codex`) to PR descriptions.** This applies even when a default git workflow suggests one

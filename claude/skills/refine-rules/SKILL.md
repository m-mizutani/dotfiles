---
name: refine-rules
description: リポジトリのコードベースを分析してClaude Code用のコーディングルール（CLAUDE.md / .claude/rules/）を生成・更新し、併せてコードベース側の不足や問題点も指摘・修正する。「ルールを作りたい」「コーディング規約を整えたい」「CLAUDE.mdを作りたい・見直したい」「.claude/rulesを整備したい」「このリポジトリのルールを書いて」「プロジェクトのコーディングスタイルを整えて」といったリクエストで使うこと。新規リポジトリの初期設定にも、既存ルール・既存コードベースの継続的な見直し・改善にも対応する。
---

# Refine Rules

リポジトリのコードベースを観察し、Claude Codeが従うべきコーディングルールを生成・更新するスキル。併せて、コードベース側の不足や問題点（壊れたビルド、緩すぎるリンター、ルールと実装の乖離など）も指摘し、合意のうえで修正する。
...
コードベースを観察する過程で、以下のような「ルール以前の問題」に気づいたら、ルールを書く前に**ユーザーに指摘し、合意のうえで修正する**こと。ルールを整備しても土台が崩れていると意味がないため。

**指摘・修正の対象：**
- ビルド・テスト・リントが壊れている、または定義されていない
- リンター/フォーマッター設定が無い、もしくは緩すぎる
- CIが無い、または主要チェックが走っていない
- `.gitignore` の漏れ（ビルド成果物・秘密情報の混入リスク）
- 既存の CLAUDE.md / `.claude/rules/` がコードベースの現状と乖離している
- 命名・ディレクトリ構成が宣言した規約と食い違っている
- README に書かれた手順が実際には動かない

**進め方：**
1. 観察で気づいた問題を箇条書きで列挙してユーザーに見せる
2. それぞれについて「ルールに書くだけ」「実際に直す」「無視する」のどれにするかを確認する
3. 直すと決めたものは、ルール生成と同じセッション内で修正コミットを作る（ルール側にも対応する規約として記述する）

「ルールを書く」ことと「リポジトリを健全な状態にする」ことはセットで進める。

...
- PART B 教訓への一次蒸留: 各エピソードから具体（このパス・このトークン形式）を
  剥がし、「Claudeが本来どう振る舞うべきだったか」を一般的な原則として一文で述べる。
  似たものはまとめる。各原則を GENERAL（どのプロジェクトでも成り立つ）か
  PROJECT-SPECIFIC（このコードベース固有）かでラベル付けする。
```

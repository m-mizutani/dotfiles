...
ユーザーに問う箇所は、会話で尋ねるだけでなく **spec 本文の該当位置に `<div class="decide">` の callout を置いて強調表示する**（書式は「統合specファイルの作成」を参照）。会話だけで問うと、spec を後から読む人にはどこが未確定なのか分からない。
...
**全体**
- [ ] ユーザーの判断が必要な箇所が `<div class="decide">` の callout で表記され、**決めること・選択肢・各選択肢を採った場合の帰結・現時点の推奨**が書かれているか。判断が必要な箇所がないなら callout が1つも残っていないか
...
- **ユーザーの判断が必要な箇所は `<div class="decide">` の callout で表記する。** テンプレートの `<style>` が強調色（琥珀）の枠線・背景と「要判断」ラベルを描画し、本文に埋もれないようにする。書く場所は判断が関わる章の該当位置（例: 保存先の選択なら「2. 基本設計 → 状態の持ち方」の直後）で、章末や文書末にまとめない。callout の中には **①何を決める必要があるのか ②選択肢（named な案として） ③各選択肢を採った場合の帰結 ④現時点での推奨と理由** を書く。ユーザーの判断が出たら callout を削除し、確定した内容を本文と「3. 設計判断」へ反映する
...
<style>
  ...
  .spec .decide { border-left: 4px solid #d98a2b; padding: .5rem .9rem; background: #d98a2b26; border-radius: 4px; margin: 1rem 0; }
  .spec .decide::before { content: "要判断"; display: block; font-weight: 700; font-size: .85em; letter-spacing: .08em; color: #c2761a; margin-bottom: .25rem; }
...
<article class="spec">
  ...
  <div class="decide">
    <strong>[決めること]</strong>
    <ul>
      <li><strong>案A: [名前]</strong> — [内容]。採った場合: [帰結]</li>
      <li><strong>案B: [名前]</strong> — [内容]。採った場合: [帰結]</li>
    </ul>
    <p>推奨: [どちらを推すか] — [理由]</p>
  </div>
  <p class="note">この <code>decide</code> callout は、ユーザーの判断が必要な箇所に置く。置き場所は判断が関わる章の該当位置であり、この3章に限らない。判断が確定したら callout を削除し、確定内容を本文と上の設計判断リストへ反映する。判断が必要な箇所がなければ callout ごと削除する。</p>
...
1. **artifacts に公開する（必須）**: spec作成（`spec.html` の書き込み）が完了したら、`Artifact` ツールに `file_path` として `.spec/{slug}/spec.html` を渡して公開する。`title` はタスク名のspec、`icon` は `document` を指定する（初回公開時のみ）。返ってきたArtifactのURLをユーザーに提示せよ。specを修正したら **同じ `file_path` で再度 `Artifact` を呼び**、同一URLへ再デプロイする（URLを変えない）。**ローカルファイルのまま提示して済ませることは禁止。**
2. **ユーザーにレビューを依頼し、ここで必ず停止する（必須）**: specを提示した状態でレビューを依頼し、**ユーザーから明示的な承認を得るまで Phase 3 に進んではならない。** ユーザーが「OK」「進めて」「LGTM」等の承認を返すまで待つこと。`decide` callout が残っている場合は、レビュー依頼の文でも**どの章のどの点について判断が必要かを一覧で示す**（spec 本文の callout と重複してよい）。

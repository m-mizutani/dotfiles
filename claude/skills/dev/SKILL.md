---
name: dev
description: "spec-driven development の計画フェーズを担う。開発タスクの spec（要求事項・基本設計・設計判断・テスト計画・詳細設計・実装計画）を作成・更新してユーザーの承認を得て、実装用の goal を出力するところまでを1つの作業とする。実装・commit・PR作成はこのスキルでは行わず `implement` skill に引き渡す。ユーザーが開発タスクについて言及したとき、feature branchでの作業を始めるとき、specを作りたい・見直したいときに必ずこのスキルを使うこと。"
allowed-tools: Read, Write, Edit, Bash, Grep, Glob, Agent, WebFetch, WebSearch, Artifact, Skill
---

# spec-driven development 計画ワークフロー

このスキルの範囲は **spec の作成・更新と、実装用 goal の出力まで**である。実装・レビュー・commit・push・PR作成は `implement` skill の担当であり、**このスキルから実装を開始してはならない**。
...
Phase 1 と Phase 2 のどちらを通った場合も、最後は必ず **Phase 3: 実装用 goal の出力** で終わる。

判定結果をユーザーに伝えてから、該当フェーズの作業を開始せよ。

> **⚠️ 絶対ルール**
> - **specファイルが存在しない場合、Phase 1 以外のフェーズに進んではならない。** どんな場合でも、まず spec を作成し、ユーザーの承認を得る
> - **このスキルは実装を行わない。** Auto modeであっても、ユーザーから「そのまま実装して」と言われた場合であっても、このスキルの中でコードを書いてはならない。実装は Phase 3 で goal を出力したうえで `implement` skill に渡す
...
# Phase 3: 実装用 goal の出力（このスキルの終点）

spec が承認された（または Phase 2 で妥当性が確認された）ら、**実装には入らず**、`implement` skill を回すための goal を組み立てて画面に出力する。**ここでこのスキルは終わる。**

## 手順

1. `implement` skill の完了条件を、この spec の内容に合わせて具体化する。具体化するのは次の3点：
   - **spec の実体パス**（`.spec/{slug}/spec.html`）
   - **PRの本数**。「6. 実装計画」で分割している場合はその本数と、各PRが含む変更
   - **この spec 固有の検証**。「4. テスト計画」から、通すべき検証（統合テスト、マイグレーションの適用確認など）を拾う
2. 組み立てた goal を、**`/goal ` で始まる1行のコードブロックとして出力する**。ユーザーがそのまま貼れる形にする（コードブロックの中に説明文を混ぜない）
3. 続けて「実装は `/implement` を実行してください」と案内する
4. 停止する

## 出力する goal の雛形

`{}` を spec の内容で埋める。分割しない場合は末尾の1文を落とす。

```
/goal .spec/{slug}/spec.html の全タスクがdoneになり、プロジェクトの検証ゲート（テスト/lint）が通り、{この spec 固有の検証} を確認し、codex-review（使えない場合はcode-review）の全指摘が「確信が強い→修正または却下済み」「確信が弱い→修正のうえ論点と判断理由を報告済み」「判断が必要→保留してユーザーに選択肢を提示済み」のいずれかで処理され、変更がcommit・pushされてPR（判断が必要な指摘が残っていればdraft）が作成され、そのCIが通り、3区分の報告とPRのURLを出し終えていること。PRは{N}本に分割し、全PRについてこれを満たして全PRのURLを報告し終えていること
```

## このフェーズの禁止事項

- **実装・commit・push・PR作成を行ってはならない。** ユーザーが「そのまま実装して」と指示した場合も、このスキルの中で実装せず `implement` skill を起動する
- goal を `ProposeGoal` ツールや `/goal` の実行で自分から設定しようとしない。**出力するだけ**にとどめる
- 実装の見通しや懸念を長々と書き添えない。goal と `/implement` の案内以外に伝えるべきことがある場合は、1〜2文にまとめる

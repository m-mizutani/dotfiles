...
## specファイルのプレビュー
- `mo` コマンドが利用可能な場合、spec作成完了時に `mo .cckiro/specs/{slug}/spec.md` を実行してブラウザでプレビューを開く
- `mo` はMarkdownをブラウザでライブプレビューするツール。バックグラウンドで起動され、ファイル保存時に自動リロードされるため、ユーザーがspecを確認・編集しやすくなる

spec作成後、ユーザーにレビューを依頼する。承認されたら **Phase 2** に進む。
...
## 実装完了時の差分確認
- すべての実装ステップが完了したら、`npx difit .` を実行して差分を確認する
